...
`α ↪ β` は、型 `α` から型 `β` への埋め込みを表す。
関数のように入力から出力を返すが、異なる入力が同じ出力へ潰れないことも保証する。
`Hand.tiles` では、手牌内の異なる位置が同じ物理牌を指さないことを表す。
...
`List.ofFn f` は、有限な添字型の各値に関数 `f` を適用し、その結果をリストとして並べる。
`Hand.tileTypes` では、手牌内の各位置から物理牌を取り出し、対応する牌種をリストにしている。
...
`proof.cons value` は、`xs.Perm ys` の証拠 `proof` から、両方の先頭へ同じ要素 `value` を加えた
