...
旧実装の分類を維持せず、現行定義を仕様として採用する。完成面子を除去した後も
同じ待ち核集合を保つ場合だけを可約とし、`1223m` はコード `[21, 26]`、
`1233m` はコード `[26, 33]` を持つ既約例として回帰テストに固定した。
現行4枚形集計は reducible 1647、irreducible 2109 である。
